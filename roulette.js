...
var outputs = {
    ...
    fail: function(message, loss, landed) { return `\`Unfortunate\` ${message.author.username}\`, it landed on \`${landed} \`and you lost **$\`${loss}\`**...\`` }
}
...
function wheel(args, ifprofile, prefix, message) {
    ...
    if (color && (color.toLowerCase() == "green" || color.toLowerCase() == "black" || color.toLowerCase() == "red") && ifprofile && typeof color === "string" && asyncUser.getUserObj(message.author).awaitRoulette) {
        ...
        if (bet <= Number(ifprofile)) {
            ...
            if (probability == 0) {
                ...
                if (color.toLowerCase() == "green") {
                    ...
                    return outputs.fail(message, bet, "\`:green_circle: Green\`");
                }
            }
        ...
        return `\`Please specify a valid color: green, red, or black (say it in the chat)!\``; 
    }
}

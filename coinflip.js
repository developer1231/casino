    ...
    if (args) {
      ...
      if (amount && side && amount > 0 && ifprofile && amount <= Number(ifprofile) && (side.toLowerCase() == "heads" || side.toLowerCase() == "tails") && typeof amount === "number") {
          ...
          if ((probability == 0 && side.toLowerCase() == "heads") || (probability == 1 && side.toLowerCase() == "tails")) {
            ...
            return "\`HOORAY!, \`" + message.author.username + "\`! You guessed correctly and gained **$\`" + amount + "\`!**\`";
          }
          else if ((probability == 1 && side.toLowerCase() == "heads") || (probability == 0 && side.toLowerCase() == "tails")) {
            
            ls.set(message.author.id + "profile", Number(ls.get(message.author.id + "profile")) - amount);
            return "\`Unlucky you \`" + message.author.username + "\`, you guessed \`" + side + " \`but the coin was\` " + coinside[probability] + "\`. You lost $\`" + amount + "\`.\`";
          }
          else {
            
            console.log("Error - Click to reveal: ``` Error in line 14 of coinflip.js; Local variable probability did not return any of the 2 possible values: coinside[0] (heads) and coinside[1] (tails). ```");
          }
      }
      else if (!ifprofile) {

...
function hangmanEnd(message) {
    if (ls.exist(message.guild.id + "hangmanGame")) {
        if (ls.getObj(message.guild.id + "hangmanGame").participants[0] == message.author.id || simp.discrim(message.member)) {
            ...
            return `${message.author.username}, you do not have sufficient permission to end/modify the current hangman game. Only the starter and admins can start/end this game!`;
        }
    }
    else {
...
function hangmanGuess(args, ifprofile, prefix, message) {
    if (ls.exist(message.guild.id + "hangmanGame") && ifprofile) {
        ...
        if (Number(limit) <= 1) {
            ls.remove(message.guild.id + "hangmanGame");
            return `Sorry ${message.author.username}, you ran out of  guesses. **Game over!**\n` + "The word was: **" + word.split("").join(" ") + "**";
        }
        else {
        ...
        if (gotLetter) {
            ...
            hObj.guesses = limit;
            ls.setObj(message.guild.id + "hangmanGame", hObj)
            return `Incorrect guess, ${message.author.username}... ` + limit + "\` is the amount of guesses left.\`";
        }
        }
    }
    else if (!ifprofile) {

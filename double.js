    ...
    if (args) {
      ...
      if (amount && amount > 0 && ifprofile && (amount <= Number(ifprofile) || ls.exist(message.author.id + "doubleGame")) && typeof amount === "number" && (amount <= 2500 || ls.exist(message.author.id + "doubleGame"))) {
          ...
          if (probability == 2 || probability == 3) {
            ...
            console.log("Error - Click to reveal: ``` Error in line 12 of double.js; Local variable probability did not return any of the 3 possible values: 0, 1, and 2. ```");
          }
      }
      else if (!ifprofile) {
        ...
        return `\`you cannot bet over $2500 on double.\``;
      }
      else {
